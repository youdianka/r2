...
class VSSBlock(nn.Module):
    def __init__(
        ...
        # 创建kwargs的副本并移除可能存在的dropout参数
        kwargs_copy = kwargs.copy()
        if 'dropout' in kwargs_copy:
            del kwargs_copy['dropout']

        self.self_attention = SS2D(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, **kwargs_copy)
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9], depths_decoder=[9, 2, 2],
                 dims=[96, 192, 384], dims_decoder=[384, 192, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
    ...
    def forward(self, x):
        ...
        return skip_list[0], skip_list[1], skip_list[2]  # 只返回3个阶段
